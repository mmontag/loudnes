...
def  main():
  ...
  installer = "\installer\LoudNES Installer.exe"
   
  if demo:
    installer = "\installer\LoudNES Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\LoudNES manual.pdf" 
  ]
  ...
  ZIPNAME = "LoudNES-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "LoudNES-v" + FULLVERSIONSTR + "-win-demo.zip"
