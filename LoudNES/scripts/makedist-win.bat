...
if %1 == 1 (echo Making LoudNES Windows DEMO VERSION distribution ...) else (echo Making LoudNES Windows FULL VERSION distribution ...)
...
REM - msbuild LoudNES-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild LoudNES.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild LoudNES.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\LoudNES.aaxplugin\Contents\Win32\LoudNES.aaxplugin --out .\build-win\aax\bin\LoudNES.aaxplugin\Contents\Win32\LoudNES.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\LoudNES.aaxplugin\Contents\x64\LoudNES.aaxplugin --out .\build-win\aax\bin\LoudNES.aaxplugin\Contents\x64\LoudNES.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\LoudNES.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\LoudNES.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "LoudNES Installer" ".\installer\LoudNES Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\LoudNES Installer.exe" ".\installer\LoudNES Demo Installer.exe"
REM -del ".\installer\LoudNES Installer.exe"
REM -)
